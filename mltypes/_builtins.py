...
from pymexutil import _check_dims
import mx
import mex
import numbers
...
class _object(mx.Array):
    ...
    The object foo is thus required to handle the entire expression. 
    Normally one would implement this sanely by doing the first item and 
    then passing the rest of the expression on to the result, but it is 
    perfectly permissible for foo to fake the whole thing. subsasgn works 
    similarly. This is insane, and at first glance I'd say that this is not 
    implementable in a sufficiently general way in Python. It doesn't help 
    that they have two different sets of indexing brackets, one of which is 
    *also* used to indicate function calls *and* is a valid target
    ...
    and I'd recommend that if you need a particular class to work a particular 
    way, that you use the mltypes package to implement it and let me know 
    how it goes. I've added an example: mltypes.containers.Map
    '''
    def __getitem__(self, key):
        if isinstance(key, tuple):
            (ind, prop) = key
    ...
    def __setitem__(self, key, val):
        if isinstance(key, tuple):
            ...
            raise KeyError, ("You'll have to set one index+prop at a "
                             "time: self[ind, field] = foo")
        ind = _check_dims(self, ind)
...
__all__ = ['cell', 'struct', 'function_handle', 
           '_numeric', '_object']
